...
volatile boolean timer10ms_flag = false;
volatile boolean timer1ms_flag = false;

static unsigned long timer10ms_count = 0;
static unsigned long timer1ms_count = 0;
...
void TMR0_IRQHandler(void)
{
  if(TIMER_GetIntFlag(TIMER0) == 1)
  {
    ...
    system_time++;
     ...
     if(++timer1ms_count >= 10)
    {
			timer1ms_flag = true;
			timer1ms_count = 0;
    }

    if(++timer10ms_count >= 100)
    {
			timer10ms_flag = true;
			timer10ms_count = 0;
    }

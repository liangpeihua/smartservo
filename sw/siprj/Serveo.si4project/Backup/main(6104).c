...
extern volatile unsigned long  timer10ms_count;
extern volatile unsigned long  timer1ms_count;
...
int32_t main(void)
{
  ...
  while(1)
  {
    ...
    if(timer1ms_count >= SERVO_DETECT_FREQ)
    {
    	timer1ms_count = 0;
			servodet_process();
    }
    
    if(timer10ms_count >= MOTOR_CTRL_FREQ)
    {
    	timer10ms_count = 0;
			motor_process();
    }

    ...
    switch(device_type)
    {
      case SMART_SERVO:
      {
        ...
        if(((report_smart_speed_mode == 0x01) &&
            (abs_user(pre_smart_servo_speed_val - g_servo_info.cur_speed/10) > 2)) ||
            (report_smart_speed_mode == 0x02))
        {
          pre_smart_servo_speed_val = g_servo_info.cur_speed/10;
          send_smart_speed(g_servo_info.cur_speed/10);
        }

...
static void read_hardware_id_response(void *arg)
{
  uint8_t checksum;
  uint16_t ad1_value, ad2_value;
  uint32_t sample_times = 16;
  uint32_t ad1_value_sum, ad2_value_sum;
  ad1_value_sum = 0;
  ad2_value_sum = 0;
  while(sample_times--)
  {
      ad1_value_sum += analogRead(SMART_SERVO_HW_Verison_D0);
      ad2_value_sum += analogRead(SMART_SERVO_HW_Verison_D1);
  }

  ad1_value = ad1_value_sum >> 4;
  ad2_value = ad2_value_sum >> 4;
  //response mesaage to UART0
  write_byte_uart0(START_SYSEX);
  write_byte_uart0(device_id);
  write_byte_uart0(CTL_GENERAL);
  write_byte_uart0(CTL_READ_HARDWARE_ID);
  checksum = (device_id + CTL_GENERAL + CTL_READ_HARDWARE_ID);
  checksum += sendShort(ad1_value, false);
  checksum += sendShort(ad2_value, false);
  checksum = checksum & 0x7f;
  write_byte_uart0(checksum);
  write_byte_uart0(END_SYSEX);
}
...
static void smart_servo_cmd_process(void *arg)
{
  ...
  switch(cmd)
  {
    ...
    case SET_SERVO_ABSOLUTE_POS:
    {
      ...
      if((speed > SMART_SERVO_PER_SPEED_MAX) || (speed < -SMART_SERVO_PER_SPEED_MAX))
      {
        speed = (float)SMART_SERVO_PER_SPEED_MAX;
      }
      servodriver_run_abs_pos(pos_value, speed);
//      uart_printf(UART0,"pos:%d ,speed %f, sysexBytesRead:%d\r\n",pos_value,speed,sysexBytesRead);
    ...
    case SET_SERVO_RELATIVE_POS:
    {
      ...
      servodriver_run_relative_pos(pos_value,speed);
      SendErrorUart0(PROCESS_SUC);
      break;
    }
    ...
    case SET_SERVO_ABSOLUTE_ANGLE:
    {
      ...
      pos_angle = round((RAW_ANGLE_MAX_INT * raw_angle)/360.0);
      speed = readShort(sysex.val.value,4,true);
      speed = constrain(speed,-SMART_SERVO_PER_SPEED_MAX,SMART_SERVO_PER_SPEED_MAX);
    ...
    case SET_SERVO_PWM_MOVE:
    {
      ...
      smart_servo_rotation((int16_t)speed_pwm);
      SendErrorUart0(PROCESS_SUC);
      break;
    }
    ...
    case SET_SERVO_INIT_ANGLE:
    {
      ...
      if(init_mode == 0)
      {
        ...
        if((abs_user(smart_servo_cur_pos) > RAW_ANGLE_MAX_INT/2) && (smart_servo_cur_pos > 0))
        {
          smart_servo_cur_pos = smart_servo_cur_pos - RAW_ANGLE_MAX_INT;
        }
        else if((abs_user(smart_servo_cur_pos) > RAW_ANGLE_MAX_INT/2) && (smart_servo_cur_pos < 0))
        {
          smart_servo_cur_pos = smart_servo_cur_pos + RAW_ANGLE_MAX_INT;
        }
    ...
    switch(device_type)
    {
      case SMART_SERVO:
      {
        if(((report_smart_pos_mode == 0x01) &&
            (abs_user(pre_smart_servo_pos_val - smart_servo_pos_val) > 5)) ||
        ...
        if(((report_smart_angle_mode == 0x01) &&
            (abs_user(pre_smart_servo_angle_val - smart_servo_angle_val) > 0)) ||
        ...
        if(((report_smart_temp_mode == 0x01) &&
            (abs_user(pre_smart_servo_temp_val - smart_servo_temp_val) > 5)) ||
        ...
        if(((report_smart_speed_mode == 0x01) &&
            (abs_user(pre_smart_servo_speed_val - smart_servo_speed_val) > 2)) ||
        ...
        if(((report_smart_voltage_mode == 0x01) &&
            (abs_user(pre_smart_servo_voltage_val - smart_servo_voltage_val) > 10)) ||
        ...
        if(((report_smart_current_mode == 0x01) &&
            (abs_user(pre_smart_servo_current_val - smart_servo_current_val) > 10)) ||
...
void device_neep_loop_in_sampling(void)
{
  ...
  {
    if((protect_flag != true) && (shake_hand_flag != true))
    {
...
static void cmd_absolute_pos(char *cmd)
{
  ...
  float speed = SMART_SERVO_PER_SPEED_MAX;
...
static void cmd_absolute_angle(char *cmd)
{
  ...
  angle = round((RAW_ANGLE_MAX_INT * angle_raw)/360.0);
  speed = constrain(speed,-SMART_SERVO_PER_SPEED_MAX,SMART_SERVO_PER_SPEED_MAX);
  servodriver_run_abs_pos((long)angle,speed);
  SendErrorUart0(PROCESS_SUC);
}

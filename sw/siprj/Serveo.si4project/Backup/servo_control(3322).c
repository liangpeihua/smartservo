...
static void motor_pos_mode(void)
{
	...
	if(s_bMotionStatusChanged)
	{
		...
		max_speed = abs_user(g_servo_info.tar_speed);
	}
	...
	max_speed = max(abs_user(g_servo_info.tar_speed),abs_user(g_servo_info.cur_speed));
	A = abs_user(g_servo_info.tar_speed);//最大目标速度
	B = 5*abs_user(max_speed)+10;//减速临街角
	K = 0.01;//系数
	pos_error = g_servo_info.tar_pos - g_servo_info.cur_pos;
	LIMIT_DEATH(pos_error, 10);
	...
	//else if(abspos_error < (abs_user(g_servo_info.tar_speed) * 5))

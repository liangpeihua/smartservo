...
//	}
	...
	if(abspos_error < 300)
	{
		target_speed = abspos_error * 0.1;//k >= 0.1
	}
	else
	{
		target_speed = abspos_error * 0.5;
	}
	...
	speed_error = constrain(speed_error,-10,10);
	speed_ctrl.integral += speed_error;
	speed_ctrl.integral = constrain(speed_ctrl.integral,-(100*MAX_OUTPUT_PWM/speed_ctrl.Ki),(10*MAX_OUTPUT_PWM/speed_ctrl.Ki));

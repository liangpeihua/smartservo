...
static int32_t current_zero_offset = 0;
static int32_t angle_pos_offset = 0;

void servodet_init(void)
{
	current_zero_offset = analogRead(SMART_SERVO_CURR_AD);
	angle_pos_offset = (int16_t)flash_read_angle_offset();
}

static void servodet_pos_handle(void)
{
	static int32_t circular_count = 0;
  ...
  pre_pos = cur_pos;
  cur_pos = cur_pos - angle_pos_offset;
  cur_pos = circular_count * RAW_ANGLE_MAX_INT + cur_pos;

  g_servo_info.pos = cur_pos * 3600 / 4096;//0.1°
}

static void servodet_speed_handle(void)
{
	int16_t pos_diff;
	long time_diff,cur_time;
	static long pre_time = 0;
	static int16_t pre_pos = 0;
	int16_t cur_pos = g_servo_info.pos;
	...
	pre_time = cur_time;
	g_servo_info.speed = (600L * pos_diff * 1000L) / (period_time * RAW_ANGLE_MAX_FLOAT);//0.1rpm / min
}
...
static void servodet_current_handle(void)
{
	...
	value = (int32_t)(value - current_zero_offset)*49/512;

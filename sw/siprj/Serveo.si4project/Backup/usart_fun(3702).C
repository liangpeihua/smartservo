...
void COMSendBuffer(uint32_t id, void *buf, uint32_t bufsize)
{
	...
	{
		UART_WRITE(UART1, g_ucCOMTxBuf[i]);
...
void USART_SendPackage(void)
{
	...
	temp1 = logab(10,count);
	temp2 = log10(count);
	
	//speed
	*(int16_t*)&buff[0] = target_speed = logab(2,count);///g_servo_info.cur_speed;
	*(int16_t*)&buff[2] = g_servo_info.cur_pos/10;
	*(int16_t*)&buff[4] = target_speed;//g_servo_info.tar_speed;
	*(int16_t*)&buff[6] = g_servo_info.tar_pos/10;
	COMSendBuffer(0x00660001, buff, 8);

	//output pwm
//	*(int16_t*)&buff[0] = speed_ctrl.output;
//	*(int16_t*)&buff[2] = pos_ctrl.output;
//	*(int16_t*)&buff[4] = s_output_pwm;
//	*(int16_t*)&buff[6] = 0;

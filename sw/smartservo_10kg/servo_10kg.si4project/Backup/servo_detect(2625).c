...
static void servodet_current_handle(void)
{
	...
	s_limit_current = (g_servo_info.voltage * s_limit_current) / 7250;//7.4V
	s_limit_current -= 0;

	if(g_servo_info.voltage < 8000)
	{
		s_limit_current = (g_servo_info.voltage * s_limit_current) / 7250;//7.4V
		s_limit_current -= 0;
	...
	{
		g_servo_info.limit_pwm = 8000 * MAX_OUTPUT_PWM / g_servo_info.voltage; //8v -> max output
		g_servo_info.limit_pwm = constrain(g_servo_info.limit_pwm, 0, MAX_OUTPUT_PWM);

		//max_I = 8000 * I_8V / U
		s_limit_current = 8000 * (8000 * s_limit_current / 7250) / g_servo_info.voltage;

...
//监测阈值
#define SHORT_CURRENT_THSD			    			2000			//1mA,驱动芯片短路保护阈值
...
{
	static uint16_t short_count = 0;
	static uint16_t over_count = 0;
	static uint8_t index = 0; 
	static int32_t tempvalue[8] = {0};
	static int32_t sum = 0;
	uint32_t limit_voltage;
	int32_t value;
	...
	s_limit_current = (-0.0102*pow(g_servo_info.temperature,3) + 
										1.7405*pow(g_servo_info.temperature,2) - \
										99.922*g_servo_info.temperature + \
										2503.6);

	limit_voltage = g_servo_info.voltage;
	if(limit_voltage > 8400){
		limit_voltage = 8400;
	}
	s_limit_current = (limit_voltage * s_limit_current) / 7250;//7.4V
	s_limit_current -= 0;

	g_servo_info.limit_pwm = MAX_OUTPUT_PWM;//8000 * MAX_OUTPUT_PWM / g_servo_info.voltage; //8.4v -> max output
	g_servo_info.limit_pwm = constrain(g_servo_info.limit_pwm, 0, MAX_OUTPUT_PWM);

//	if(g_servo_info.voltage < 8400)
//	{
//		s_limit_current = (limit_voltage * s_limit_current) / 7250;//7.4V
//		s_limit_current -= 0;
//		g_servo_info.limit_pwm = MAX_OUTPUT_PWM; 
...
//		s_limit_current -= 0;
//		
//		g_servo_info.limit_pwm = 8400 * MAX_OUTPUT_PWM / g_servo_info.voltage; //8.4v -> max output
//		g_servo_info.limit_pwm = constrain(g_servo_info.limit_pwm, 0, MAX_OUTPUT_PWM);
...
//	}
	...
	if(abs_user(value) > s_limit_current)
	{
		over_count++;
		if(over_count>200)//2s
		{
	    over_count = 200;
	   // g_servo_info.errorid |= OVER_CURRENT_ERR;
		}      
	...
	{
		over_count = 0;
		if(value < (s_limit_current-CURR_HYSTERESIS_THSD))
		{
		 // g_servo_info.errorid &= ~(OVER_CURRENT_ERR);
		}
	}

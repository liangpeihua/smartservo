...
#define LIMIT_MAX_VOLTAGE		8000		
	...
	else
	{
		...
		g_servo_info.limit_pwm = LIMIT_MAX_VOLTAGE * MAX_OUTPUT_PWM / g_servo_info.voltage; //8v -> max output
		g_servo_info.limit_pwm = constrain(g_servo_info.limit_pwm, 0, MAX_OUTPUT_PWM);

		//功率守恒
		//Ix = LIMIT_MAX_VOLTAGE * I_8V / Ux
		s_limit_current = (LIMIT_MAX_VOLTAGE * s_limit_current) / 7250;//7.4V
		s_limit_current -= 0;
		s_limit_current = LIMIT_MAX_VOLTAGE * s_limit_current / g_servo_info.voltage;
	}

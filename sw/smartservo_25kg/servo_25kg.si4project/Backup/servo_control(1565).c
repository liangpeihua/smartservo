...
static void motor_pos_mode(void)
{
	...
	if(pre_tar_pos != g_servo_info.tar_pos)
	{
		...
		max_tar_speed = abs_user(pos_error * 20 / 50);//50°时，允许最大目标速度为20
		max_tar_speed = constrain(max_tar_speed,-abs_user(g_servo_info.tar_speed),abs_user(g_servo_info.tar_speed));

...
static void motor_pos_mode(void)
{
	...
	if(s_bMotionStatusChanged)
	{
		...
		break_speed = abs_user(g_servo_info.cur_speed);
	...
	if(pre_tar_pos != g_servo_info.tar_pos)
	{
		...
		pre_tar_pos = g_servo_info.tar_pos;
		stop_flag = false;
		break_speed = min(abs_user(pos_error/5), abs_user(g_servo_info.tar_speed));
	}
	...
	B = 5*abs_user(break_speed);//减速临界角
	K = 0.002;//系数
	pos_error = g_servo_info.tar_pos - g_servo_info.cur_pos;
	...
	else
	{
		...
		//break_speed = (break_speed*99 + abs_user(g_servo_info.tar_speed)*1) / 100;//waring
	}

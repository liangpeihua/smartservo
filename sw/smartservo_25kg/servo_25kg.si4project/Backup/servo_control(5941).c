...
static void motor_pos_mode(void)
{
	...
	static int32_t max_tar_speed = 0;
	int32_t A;//最大目标速度
	int32_t B;//减速临界角
	float K;//系数
	...
	if(s_bMotionStatusChanged)
	{
		...
		break_speed = g_servo_info.cur_speed;
		pre_tar_pos = g_servo_info.tar_pos;
		max_tar_speed = abs_user(g_servo_info.tar_speed);
	}
	...
	if(pre_tar_pos != g_servo_info.tar_pos)
	{
		...
		max_tar_speed = abs_user(pos_error * 20 / 50);//速度为50时，允许最小减速临界角为30°
		max_tar_speed = constrain(max_tar_speed,10,MAX_TAR_SPEED);
		if(abs_user(g_servo_info.cur_speed) < max_tar_speed)
		{
			max_tar_speed = abs_user(g_servo_info.cur_speed);
		}
		break_speed = (abs_user(max_tar_speed)*5 + abs_user(g_servo_info.cur_speed)*5) /10;//waring
	}
	...
	//speed pid
	A = abs_user(max_tar_speed);//最大目标速度
	...
	{
		target_speed = abs_user(max_tar_speed);
		break_speed = (abs_user(max_tar_speed)*5 + abs_user(g_servo_info.cur_speed)*5) /10;//waring
	}
	
	target_speed = constrain(target_speed,0,abs_user(max_tar_speed));
